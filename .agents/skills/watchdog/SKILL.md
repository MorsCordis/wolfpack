...
**Fail loud when you cannot execute.** If your seat cannot actually run the mandated tests within its execution constraints (print-mode timeout shorter than the suite, cannot block on a long subprocess, no test DB), do NOT emit a `test_result` score and do NOT claim a run happened — write `test_result: BLOCKED` in certification.md, state exactly what you could not run and why, and stop for the operator. Asserting a pass you did not observe is a certification-invalidating defect (two such fabrications were caught and reverted on soft-delete-unification, 2026-08-18). **Sanctioned alternative — audit-of-evidence:** the operator runs the suite on the HOST and pre-stages the raw log plus an evidence summary in the plan dir (e.g. `tracker-full-sweep.log`, `tracker-test-evidence.md`); your job is then a forensic audit of that log — verify the run count, match every failure against the known-broken baseline file, and check the triage — citing the log, not executing. A certification that names neither a run it performed nor pre-staged evidence it audited is hollow and must not score `test_result`. Write `certification.md` / `smoke-tests.md` into the plan dir by absolute path, never relative to CWD (a containerized seat's CWD is the worktree root and artifacts get stranded there).
...
```json
{
  ...
  "timing": "WRITTEN BY THE AGGREGATOR — do NOT hand-author. node scripts/wolfpack-timing.mjs fills { total_s, duration, total_method, by_phase, by_model, complete, incomplete_phases? } from timing.jsonl. See the aggregator section below.",
  "parallelism_metrics": {
...
```bash
node scripts/wolfpack-timing.mjs "$PLAN_DIR"
```
